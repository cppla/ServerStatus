...
int CMain::HandleMessage(int ClientNetID, char *pMessage)
{
    ...
    if (str_comp_num(pMessage, "update", sizeof("update") - 1) == 0)
    {
        ...
        if (rStart["type"].type)
            str_copy(pClient->m_Stats.m_Type, rStart["type"].u.string.ptr, sizeof(pClient->m_Stats.m_Type));
        ...
        if (m_Config.m_Verbose)
        {
            if (rStart["online4"].type)
                dbg_msg("main", "Online4: %s\nUptime: %" PRId64 "\nType: %" PRId64 "\nLoad_1: %f\nLoad_5: %f\nLoad_15: %f\nPing_10010: %f\nPing_189: %f\nPing_10086: %f\nTime_10010: %" PRId64 "\nTime_189: %" PRId64 "\nTime_10086: %" PRId64 "\nTcp_count: %" PRId64 "\nUdp_count: %" PRId64 "\nprocess_count: %" PRId64 "\nthread_count: %" PRId64 "\nNetworkRx: %" PRId64 "\nNetworkTx: %" PRId64 "\nNetworkIN: %" PRId64 "\nNetworkOUT: %" PRId64 "\nMemTotal: %" PRId64 "\nMemUsed: %" PRId64 "\nSwapTotal: %" PRId64 "\nSwapUsed: %" PRId64 "\nHDDTotal: %" PRId64 "\nHDDUsed: %" PRId64 "\nCPU: %f\n",
                        ...
                        pClient->m_Stats.m_Uptime, pClient->m_Stats.m_Type,
        ...
        for (int i = 0; i < NET_MAX_CLIENTS; i++)
        {
            ...
            if (pClients[i].m_Connected)
            {
                ...
                str_format(pBuf, sizeof(aFileBuf) - (pBuf - aFileBuf),
                           ...
                           pClients[i].m_aName, pClients[i].m_Stats.m_Type, pClients[i].m_aHost, pClients[i].m_aLocation,
            ...
            else
            {
                ...
                str_format(pBuf, sizeof(aFileBuf) - (pBuf - aFileBuf), "{ \"name\": \"%s\", \"host\": \"%s\", \"location\": \"%s\", \"online4\": false, \"online6\": false, \"last_network_in\": %" PRId64 ", \"last_network_out\": %" PRId64 " },\n",
                           pClients[i].m_aName, pClients[i].m_aHost, pClients[i].m_aLocation, pClients[i].m_LastNetworkIN, pClients[i].m_LastNetworkOUT);
                pBuf += strlen(pBuf);
            }
        }
        ...
        for (unsigned i = 0; i < rStart.u.array.length; i++)
        {
            ...
            if (m_Config.m_Verbose)
            {
                if (Client(ID)->m_Disabled)
                    dbg_msg("main", "[#%d: Name: \"%s\", Username: \"%s\", Host: \"%s\", Location: \"%s\", Password: \"%s\", MonthStart: %\" PRId64 \"]",
                            ID, Client(ID)->m_aName, Client(ID)->m_aUsername, Client(ID)->m_aHost, Client(ID)->m_aLocation, Client(ID)->m_aPassword, Client(ID)->m_aMonthStart);
                else
                    dbg_msg("main", "#%d: Name: \"%s\", Username: \"%s\", Host: \"%s\", Location: \"%s\", Password: \"%s\", MonthStart: %\" PRId64 \"",
                            ID, Client(ID)->m_aName, Client(ID)->m_aUsername, Client(ID)->m_aHost, Client(ID)->m_aLocation, Client(ID)->m_aPassword, Client(ID)->m_aMonthStart);
            }
